...
export default function Registersubview() {
  ...
  return (
    ...
    <TextInput  style={styles.nameInput1} placeholder='Email'  onChangeText={(text) => {
          setemail(text);
          validateemail(text); 
        }} />
    <MaterialIcons name="email" size={24} color="black" style={styles.email} />
    {!isgmail &&(
      <Text style={styles.emailvalid} > email is not valid </Text>
    )}
    <TextInput  style={styles.nameInput3} placeholder='User Name'   onChangeText={(text) => setname(text)}/>
    <Ionicons name="person-circle" size={24} color="black" style={styles.name} />
